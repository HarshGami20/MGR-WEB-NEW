 ...
 *
 * The `--yes` flag (or `CONFIRM=yes` env var) is required.
 */
...
async function syncProductStockFromVariants(productId, db = prisma) {
...
function buildInventoryLogRows({ targetQty, branchIds, variants, simpleProductIds }) {
  const notes = `Stock seed — set to ${targetQty}`;
  const rows = [];
  ...
  for (const branchId of branchIds) {
    ...
    for (const productId of simpleProductIds) {
      rows.push({
        productId,
        variantId: null,
        branchId,
...
async function createLogsInBatches(tx, rows) {
